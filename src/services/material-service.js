...
(function() {
  ...
  // 注意：「需要」「做」「帮我做」属于生图动词，不在搜索动词里，避免「需要一张海报」走搜索路径
  const VERB = /(找|搜|搜索|检索|来个|来一?个|要个|要一?个|有没有|有木有|帮我找|调取|查一?下)/;
  // 明确的生图/制作动词：命中时走生图路径，不走搜索（优先级高于 VERB）
  const GEN_VERB = /(帮我做|帮我画|帮我设计|帮我生成|帮我出|做一?张|画一?张|设计一?张|生成一?张|出一?张|制作一?张|我需要一?张|我要一?张)/;
  ...
  Cls.prototype.looksLikeSearch = function(text) {
    ...
    // 含明确生图动词时，不走搜索（「帮我做一张」「我需要一张」应触发生图）
    if (GEN_VERB.test(text)) return false;
